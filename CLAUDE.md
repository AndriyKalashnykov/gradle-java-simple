...
- **docker** job: **runs on `v*` tag pushes ONLY** — the whole job (build the scan image → Trivy image scan → FIPS smoke test via `make image-smoke-test` → push → `cosign sign`) fires at release-cut. Job-level gate is a publishing-job form: `startsWith(github.ref, 'refs/tags/')` AND each prerequisite `== 'success'` (so a skipped/failed prereq can never let a release publish). `push` (to `ghcr.io/andriykalashnykov/gradle-java-simple/gradle-java-fips-test`) and `cosign sign` (iterates every tag from `metadata-action`, signs by digest) also keep their step-level tag gates (now redundant with the job gate, kept as minimal-diff). Uses `${{ secrets.GITHUB_TOKEN }}` for GHCR login (no user-configured secrets needed); requires `id-token: write` for cosign keyless OIDC and `packages: write` for GHCR publish. `needs: [changes, static-check, build, test, integration-test]`. **Tradeoff:** image build + Trivy image/misconfig scan + FIPS smoke test no longer run on PRs or `main` pushes — that coverage moved from per-push to release-only, so a broken Dockerfile, a FIPS-runtime regression, or a fresh base-image CVE now surfaces at the next tag (the release fails and no bad image ships) rather than on the introducing commit.
...
- [ ] **Next release (`v*` tag) will be BLOCKED on the `docker`/Trivy step by UBI9 base CVEs** — glib2 `CVE-2026-58016` (fixed `2.68.4-19.el9_8.2`) + libacl `CVE-2026-54369` (fixed `2.4.0-1.el9_8`), both HIGH, flagged since 2026-07-23. The runtime Dockerfile already runs `microdnf update -y`, so the fix is *pending in the base repos* (Trivy's advisory DB leads the UBI9 el9_8.2 stream). Since the `docker` job is now tag-only, these no longer block PRs but WILL fail the next release. **Deferred** to a Renovate `icr.io/appcafe/ibm-semeru-runtimes` base-digest bump (or a re-based el9_8.2 stream). **Before the next tag:** re-scan; if still flagged, either bump the base digest to one carrying the patched packages, or add a time-boxed `.trivyignore` waiver (`exp:` + reachability note per `security.md`). Check: the `docker` job's Trivy step on a probe tag / next release run.
...
- [x] ~~**`actions/cache@v4` Node-20 deprecation (transitive via `aquasecurity/trivy-action`)**~~ — Resolved 2026-06-25. `trivy-action@0.36.0` already bundles `actions/cache@v5.0.5` (Node 24) internally — the 0.35→0.36 bump carried the cache v4→v5 upgrade. Verified zero `running with Node 24 by default` deprecation lines across CI runs (PR #70/#71 + post-merge main). The separate `dorny/paths-filter` Node-20 source was fixed in PR #71 (v3→v4.0.1).
