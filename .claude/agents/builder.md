...
- Full build: `make build` (runs `check-env` first via sdkman)
- Clean: `make clean` or `./gradlew clean && rm -rf .gradle build app/build`
- Direct: `./gradlew clean build`

## Build Prerequisites

- Java 21 (Temurin via sdkman, auto-download via foojay-resolver)
- Gradle 9.x (sdkman)
...
1. **Toolchain not found**: foojay-resolver should auto-download. If it fails, run `sdk install java 21-tem`
